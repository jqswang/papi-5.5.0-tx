...
/** @defgroup papi_data_structures PAPI Data Structures */
   ...
   struct _perf_branch_entry { // This is from perf_event.h
   	uint64_t	from;
   	uint64_t	to;
   ...
   typedef struct _sample_data_t {
     ...
     uint32_t pid;
     uint64_t time;
     //uint64_t nr; //The number of entries in callchain (for ips)
     //uint64_t ips[16]; //let's just assume maximum 16
     uint64_t bnr; //The number of entries in branch stack (for lbr)
     struct _perf_branch_entry lbr[16];  //at least in broadwell, the number is 16
     uint64_t weight;
     uint64_t src;
     uint64_t tran;
   } sample_data_t;
   ...
   typedef void (*PAPI_overflow_handler_t) (int EventSet, void *address,
                                 sample_data_t sample_data,  long long overflow_vector, void *context);
