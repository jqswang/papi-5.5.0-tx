			       ...
			       int count, hwd_context_t *ctx )
{
   ...
   for( i = 0; i < count; i++ ) {
      ...
      if (strstr(aName, "COMMIT") != NULL)
        pe_ctl->events[i].attr.precise_ip=0;
...
/* This function read the sampling data into the buffer buf with the size sz.
It returns 0 if everything is okay.
*/
static int read_buffer(pe_event_info_t *hw, void *buf, size_t sz) {
  struct perf_event_mmap_page *hdr = hw->mmap_buf;
  size_t mask = hw->mask;
  void *data;
  unsigned long tail;
  size_t avail_sz, m, c;
  ...
  data = ((void *)hdr)+ getpagesize();

  // position of tail within the buffer payload
  tail = hdr->data_tail & mask;

  // size of what is available,
  // data_head, data_tail never wrap around
  avail_sz = hdr->data_head - hdr->data_tail;
  rmb(); // Add a memory barrier after reading the data head
  if (sz > avail_sz) //Request more than what can be offered
    return -1;

  // sz <= avail_sz, we can satisfy the request

  // c = (size till end of buffer)
  // buffer payload size is necessarily a power of two, so we can do:
  c = mask + 1 -  tail;

  m = c < sz ? c : sz; // min(c, sz)
...
_pe_dispatch_timer( int n, hwd_siginfo_t *info, void *uc)
{
  ...
  memset(&sample_data, 0, sizeof(sample_data_t));
  ...
  if ( ( thread->running_eventset[cidx]->state & PAPI_PROFILING ) &&
    ...
    char buff_tmp[1024]; // This buffer size should be enough, right?
    //Read the header
    if (read_buffer(pe, buff_tmp, sizeof(struct perf_event_header)) !=0){
    ...
    //Read until bnr
    if (read_buffer(pe, buff_tmp, 48) !=0){
      /* Restart the counters */
      if (ioctl( fd, PERF_EVENT_IOC_REFRESH, PAPI_REFRESH_VALUE ) == -1) {
        PAPIERROR( "overflow refresh failed", 0 );
      }
    ...
    ip = *( uint64_t * ) ( buff_tmp + 0 );
    sample_data.pid = *( uint32_t * ) ( buff_tmp + 8 );
    sample_data.tid = *( uint32_t * ) ( buff_tmp + 12 );
    sample_data.time = *( uint64_t * ) ( buff_tmp + 16 );
    sample_data.data_addr = *( uint64_t * ) ( buff_tmp + 24 );
    sample_data.cpu = *(uint32_t *) ( buff_tmp + 32 );
    sample_data.bnr = *( uint64_t * ) ( buff_tmp + 40 );

    //Read the branch entry
    if (read_buffer(pe, buff_tmp, sizeof(struct perf_branch_entry)*sample_data.bnr) != 0){
      /* Restart the counters */
      if (ioctl( fd, PERF_EVENT_IOC_REFRESH, PAPI_REFRESH_VALUE ) == -1) {
        PAPIERROR( "overflow refresh failed", 0 );
      }
    ...
    memcpy(sample_data.lbr,  buff_tmp, sizeof(struct perf_branch_entry) * sample_data.bnr);

    //Read the remaining
    if (read_buffer(pe, buff_tmp, 24) != 0){
      /* Restart the counters */
      if (ioctl( fd, PERF_EVENT_IOC_REFRESH, PAPI_REFRESH_VALUE ) == -1) {
        PAPIERROR( "overflow refresh failed", 0 );
      }
    ...
    sample_data.weight = *( uint64_t * ) ( buff_tmp + 0);
    sample_data.src = *( uint64_t * ) ( buff_tmp + 8 );
    sample_data.tran = *( uint64_t * ) ( buff_tmp + 16 );
...
_pe_set_overflow( EventSetInfo_t *ESI, int EventIndex, int threshold )
{
  ...
  ctl->events[evt_idx].attr.size = sizeof(ctl->events[evt_idx].attr);
  ...
  case WAKEUP_MODE_COUNTER_OVERFLOW:
    ...
    ctl->events[evt_idx].attr.sample_type = PERF_SAMPLE_IP | PERF_SAMPLE_ADDR | PERF_SAMPLE_CPU | PERF_SAMPLE_TID | PERF_SAMPLE_TIME | PERF_SAMPLE_WEIGHT | PERF_SAMPLE_DATA_SRC | PERF_SAMPLE_TRANSACTION | PERF_SAMPLE_BRANCH_STACK;
    ctl->events[evt_idx].attr.branch_sample_type = PERF_SAMPLE_BRANCH_USER | PERF_SAMPLE_BRANCH_ANY_CALL | PERF_SAMPLE_BRANCH_ANY_RETURN  | PERF_SAMPLE_BRANCH_ABORT_TX ;
