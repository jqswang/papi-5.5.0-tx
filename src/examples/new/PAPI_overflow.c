...
void
handler(int EventSet, void *address, sample_data_t sample_data, long long overflow_vector, void *context)
{
   int i;
   fprintf(stderr, OVER_FMT, EventSet, address, overflow_vector);
   fprintf(stderr, "%d branches recorded:\n", sample_data.bnr);
   for (i=0; i < sample_data.bnr; i++){
     fprintf(stderr, "FROM:0x%lx TO:0x%lx MISPRED:%u PREDICTED:%u IN-TX:%u IN-ABORT:%u\n",
     sample_data.lbr[i].from,
     sample_data.lbr[i].to,
     1U & sample_data.lbr[i].mispred, // only 1 bit
     1U & sample_data.lbr[i].predicted, // only 1 bit, Support for mispred and predicted is optional; if not supported, both values will be 0.
     1U & sample_data.lbr[i].in_tx, // only 1 bit, The branch was in a transactional memory transaction.
     1U & sample_data.lbr[i].abort // only 1 bit, The branch was in an aborted transactional memory transaction.
    );
  }
   fprintf(stderr,"\n");
   total++;
